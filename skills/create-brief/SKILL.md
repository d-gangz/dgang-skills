...
description: >
  ...
  standalone too. Triggers: "draft a brief", "shape this into a
...
`/brief` produces the structured handoff.
...
1. **Read the conversation.** Pull every signal about the task from the recent
   ...
   provided (e.g., `/brief I want to draft a follow-up email...`), treat
...
4. **Ask one focused question per weak slot.** Use `AskUserQuestion`/`request_user_input`. Each
