...
This is a practical rubric for designing CLI interfaces (args/flags/subcommands/help/output/errors/config). Design for AI agents first (composable, parseable, non-interactive), with human-friendly output as a secondary concern.

## Foreword

- CLI still uniquely powerful: inspect/control systems; works interactively and in automation.
- Modern CLI for AI agents = composable, parseable, non-interactive by default.
- Goal: maximize utility for AI automation; keep human-readable output as fallback.
...
- Optimize for AI agents by default: parseable output, clear exit codes, no interactive prompts.
- Provide `--json` for structured output; human-readable text as default for occasional human use.
- Don't leak developer-only output; reserve for `--verbose`/`--debug`.
...
- Provide safe `--dry-run`/preview; show intermediate state; require `--force` for scary actions.
...
- AI agents first: provide parseable, stable output by default.
- Offer `--json` for structured data; human-readable text is the default (no fancy formatting that breaks parsing).
...
- **No prompts by default** (AI agents cannot respond to them—commands just hang).
- If required input is missing, fail immediately with an actionable error message.
- For optional human interactivity, gate behind `--interactive` flag.
- Destructive operations: require `--force` or `--yes` flag instead of interactive confirmation.
